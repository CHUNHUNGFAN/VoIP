...
def construct_icmp_echo_packet(id, size, ipv6): 
   ...
   checksum = make_checksum(packet)    # make checksum 
...
def make_checksum(packet): 
   """THE RFC792 states: 'The 16 bit one's complement of the one's complement sum of all 16 bit words in the header.' 
...
def pingNode_function(alive=0, timeout=1.0, ipv6=0,number=sys.maxint, node=None,flood=0, size=ICMP_DATA_STR, ipv4=0): 
   ...
   if ipv6: 
        if socket.has_ipv6: 
           try: 
              info= socket.getaddrinfo(node,None) 
              host = info[0][4][0] 
              # do not print ipv6 twice if ipv6 address given as node 
              if ipv4:
                noPrintIPv6adr = 1 
           except: 
               error_report("cannot resolve %s: Unknow host" % node)
           if noPrintIPv6adr == 1:
           ...
           error_report("No support for IPv6 on this plattform") 
   ...
   if not ipv6: 
       if int(string.split(host, ".")[-1]) == 0: 
           error_report("No support for network ping") 

   # do some sanity check 
   if number == 0: 
       error_report("Invalid count of packets to transmit: '%s'" % str(a)) 
   if alive: 
       number = 1 
   ...
   if not alive: 
       if ipv6: 
           print "This is IPv6 packets"
           
           # do not print the ipv6 twice if ip adress given as node 
           if noPrintIPv6adr == 1: 
               # add 40 (header) + 8 (icmp header) + payload 
               print "PING %s : %d data bytes (40+8+%d)" % (str(node),40+8+size, size) 
           else: 
               # add 40 (header) + 8 (icmp header) + payload 
               print "PING %s (%s): %d data bytes (40+8+%d)" % (str(node),str(host), 40+8+size, size) 
       else: 
           print "This is IPv4 packets"
           # add 20 (header) + 8 (icmp header) + payload 
           print "PING %s (%s): %d data bytes (20+8+%d)" % (str(node), str(host),20+8+size, size) 
       ...
       while start <= number: 
          ...
          # create the IPv6/IPv4 socket 
           if ipv6: 
            ...
            try:
              pingSocket = socket.socket(socket.AF_INET6, socket.SOCK_RAW,socket.getprotobyname("ipv6-icmp")) 
            except socket.error, e: 
              print "socket error: %s" % e 
              error_report("You must be root (uses raw sockets)" % os.path.basename(sys.argv[0])) 
           ...
           # IPv4 
               ...
               try: 
                   ...
                   print "socket error: %s" % e 
                   error_report("You must be root (%s uses raw sockets)" % os.path.basename(sys.argv[0])) 
                
           packet = construct_icmp_echo_packet(start, size, ipv6) # make a ping packet 

           # send the ping 
           #pdb.set_trace()
           try:
                pingSocket.sendto(packet,(node,0)) 
           except socket.error, e: 
                error_report("socket error: %s" % e) 
           #pdb.set_trace()
           # reset values 
           pong = ""; iwtd = [] 
   ...
   # compute and print some status 
   # stddev computation based on ping.c from FreeBSD 
   if start != 0 or lost > 0:  # do not print status if 0 packet sent 
       ...
       if not alive: 
           ...
           if plost != 100: 
               print "round-trip min/avg/max/stddev = %.3f/%.3f/%.3f/%.3f ms" % (mint*1000, (tsum/start)*1000, maxt*1000, math.sqrt(vari)*1000) 
...
if __name__ == '__main__': 
   ...
   if map(int, version) < [2, 3]: 
      error_report("You need Python ver 2.3 or higher to run!") 

   try: 
       # opts = arguments recognized, 
       # args = arguments NOT recognized (leftovers) 
       opts, args = getopt.getopt(sys.argv[1:-1], "hat:6c:fs:4",["help", "alive", "timeout=", "ipv6","count=", "flood", "packetsize=", "ipv4"]) 
   except getopt.GetoptError: 
       # print help information and exit: 
       error_report("illegal option(s) -- " + str(sys.argv[1:])) 
   ...
   if args: 
       error_report("illegal option -- %s" % str(args)) 
        
   # default variables 
   alive = 0; timeout = 1.0; ipv6 = 1; count = sys.maxint; 
   flood = 0; size = ICMP_DATA_STR ; ipv4=0
   ...
   for o, a in opts: 
       ...
       if o == "-t" or o == "--timeout": # timeout before "lost" 
           try: 
               ...
               error_report("invalid timout: '%s'" % str(a)) 
       ...
       if o == "-s" or o == "--packetsize":  # set the ping payload size 
           try: 
               ...
               error_report("invalid packet size: '%s'" % str(a)) 
   ...
   # here we send 
   pingNode_function(alive=alive, timeout=timeout, ipv6=ipv6, number=count,node=node, flood=flood, size=size,ipv4=ipv4)
